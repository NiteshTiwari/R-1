# July 2016
# forecasting workflow
# mkil
# ARMA_model.R --> model estimaton
# ARMA_TEST.R --> rolling backtesting
# calculate mean average deviations
# calculate standard deviations

rm(list=ls())

# setwd("~/Dropbox/R/AssetPriceForecasting")
# this.dir <- ("~/Dropbox/R/AssetPriceForecasting")
setwd('C:/Users/Michael Kilchenmann/Dropbox/R/AssetPriceForecasting/')
this.dir <- ('C:/Users/Michael Kilchenmann/Dropbox/R/AssetPriceForecasting/')


start <- print(Sys.time())
del.sat <- 1
add.data <- 0
assets.name <- cbind(c("EUR/USD","USD/JPY","GBP/USD", "USD/CHF", "AUD/USD", "USD/CAD", 
                       "EUR/CHF" , "EUR/GBP", "NZD/USD", "EUR/SEK", "EUR/NOK", "USD/SEK",
                       "USD/NOK", "EUR/JPY", "XAU/USD","XAG/USD","XPD/USD","XPT/USD"))

assets.source <- cbind(c("Oanda","Oanda","Oanda", "Oanda", "Oanda", "Oanda", "Oanda", "Oanda",
                         "Oanda", "Oanda","Oanda", "Oanda", "Oanda", "Oanda", "Oanda", "Oanda",
                         "Oanda","Oanda"))

assets <- cbind(assets.name,assets.source)
forecasts <- matrix(nrow=nrow(assets),ncol=11)
colnames(forecasts) <- c("Date","Time","Model","Asset", "Forecast","Last Input Price", "AR Alpha","Rolling Hit-Rate", "SD", "MAD","ACD")
intraday <-0
pt <- 0
# Sys.Date()

for (j in 1:nrow(assets)){
  MA <- 0
  AR <- 1
  ARMA <- 0
  asset <- assets[j,1]
  if (assets[j,2]=="Oanda"){
    oanda <- 1
    google <- 0
    quandl <- 0
    strSign="n.a."
    source("ARMA_model.R")
    forecasts[j,5] <- strSign
    forecasts[j,6] <- tail(sc.data,1)
    forecasts[j,7] <- round(data.fit$coef[1],2)
    forecasts[j,11] <- round(data.pred$pred,4)
    try(source("ARMA_TEST.R"))
    }
  forecasts[j,1] <- as.character.Date(Sys.Date())
  forecasts[j,2] <- substr(as.character.Date(Sys.time()),12,19)
  forecasts[j,3] <- "AR"
  forecasts[j,4] <- asset
  # forecasts[j,4] <- round(data.pred$pred,4)
  forecasts[j,8] <- round(100*HitRate,1)
  forecasts[j,9] <- d.sd
  forecasts[j,10] <- d.mean
}
write.table(forecasts, file=paste(this.dir,"/forecasts/","forecasts2016",".csv",sep=""),append=TRUE,col.names=FALSE,sep=",",row.names = FALSE)

for (j in 1:nrow(assets)){
  MA <- 1
  AR <- 0
  ARMA <- 0
  asset <- assets[j,1]
  strSign="n.a."
  if (assets[j,2]=="Oanda"){
    oanda <- 1
    google <- 0
    quandl <- 0
    source("ARMA_model.R")
    forecasts[j,5] <- strSign
    forecasts[j,6] <- tail(sc.data,1)
    forecasts[j,11] <- round(data.pred$pred,4)
    try(source("ARMA_TEST.R"))
  }
  forecasts[j,1] <- as.character.Date(Sys.Date())
  forecasts[j,2] <- substr(as.character.Date(Sys.time()),12,19)
  forecasts[j,3] <- "MA"
  forecasts[j,4] <- asset
  # forecasts[j,4] <- round(data.pred$pred,4)
  forecasts[j,7] <- "n.a."
  forecasts[j,8] <- round(100*HitRate,1)
  forecasts[j,9] <- d.sd
  forecasts[j,10] <- d.mean
}

write.table(forecasts, file=paste(this.dir,"/forecasts/","forecasts2016",".csv",sep=""),append=TRUE,col.names=FALSE,sep=",",row.names = FALSE)

for (j in 1:nrow(assets)){
  MA <- 0
  AR <- 0
  ARMA <- 1
  asset <- assets[j,1]
  strSign="n.a."
  if (assets[j,2]=="Oanda"){
    oanda <- 1
    google <- 0
    quandl <- 0
    source("ARMA_model.R")
    forecasts[j,5] <- strSign
    forecasts[j,6] <- tail(sc.data,1)
    forecasts[j,11] <- round(data.pred$pred,4)
    try(source("ARMA_TEST.R"))
  }
  forecasts[j,1] <- as.character.Date(Sys.Date())
  forecasts[j,2] <- substr(as.character.Date(Sys.time()),12,19)
  forecasts[j,3] <- "ARMA"
  forecasts[j,4] <- asset
  # forecasts[j,4] <- round(data.pred$pred,4)
  forecasts[j,7] <- "n.a."
  forecasts[j,8] <- round(100*HitRate,1)
  forecasts[j,9] <- d.sd
  forecasts[j,10] <- d.mean
}
write.table(forecasts, file=paste(this.dir,"/forecasts/","forecasts2016",".csv",sep=""),append=TRUE,col.names=FALSE,sep=",",row.names = FALSE)

end <- print(Sys.time())
print(end-start)

# Additional Code
# write.table(data, "clipboard", sep="\t", row.names=TRUE)
# typeof(data)
# row.names(data) = as.Date(data$row.name, format = '%Y-%d-%m')
# attributes(data)
# write.table(cbind(data[,0],data[,1]), "clipboard", sep="\t", row.names=TRUE)
# write.zoo(data, "clipboard", sep="\t", index.name="Date")write.zoo(data, "clipboard", sep="\t", index.name="Date")